...
local function getActivityState()
	...
	if not scriptObj then
		return ActivityStates.DEVELOPING
	end
...
local function getWorkspaceName()
	...
	return if success and name and #name > 0 then name else "Unknown Workspace"
end
...
local function collectActivityData()
	...
	if scriptObj and activity == ActivityStates.EDITING then
		...
		if lineCount > 0 then
			details = sanitizeText(string.format("%s (%d lines)", scriptDisplay, lineCount), 128)
		else
			details = sanitizeText(scriptDisplay, 128)
		end
	else
		details = sanitizeText(activity, 128)
		scriptType = ScriptTypes.DEVELOPING
	end

	state = sanitizeText(string.format("Workspace: %s", workspaceName), 128)
...
function FroststrapStudioRPC.Initialize()
	...
	if Plugin then
		...
		Button = Toolbar:CreateButton("RPC", "Toggle Rich Presence logging", "rbxassetid://111400040119373")
